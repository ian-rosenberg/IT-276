...
Collision entity_scan_hit(Entity *self,Vector2D start,Vector2D end, CollisionFilter filter)
{
    ...
    c = gf2d_collision_trace_space(GetPlayerEntity()->currentMap, start, end ,filter);
...
int entity_platform_end_check(Entity *self)
{
    ...
    collisionList = gf2d_collision_check_space_shape(self->currentMap->mapSpace, s,filter);
...
int entity_wall_check(Entity *self, Vector2D dir)
{
    ...
    if (collisionList != NULL)
    {
        ...
        for (i = 0;i < count;i++)
        {
            ...
            if (!c)continue;
            if (!c->shape)continue;
            gf2d_shape_draw(*c->shape,gf2d_color(255,255,0,255), GetCameraPosition());
        }
        gf2d_collision_list_free(collisionList);
        return 1;
    }
...
List *entity_get_clipped_entities(Entity *self,Shape s, Uint32 layers, Uint32 team)
{
    ...
    filter.team = team;
    return gf2d_collision_check_space_shape(self->currentMap->mapSpace, s,filter);
}
...
void entity_clear_all_but_player()
{
    ...
    {
        if (ent == GetPlayerEntity())continue;
        EntityFree(ent);
    }
}
